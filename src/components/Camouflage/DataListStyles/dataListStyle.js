...
/**
 * Le layout d'un item individuel non visible, dans la liste de données
 * (utile en mode vertical_one_by_one et horizontal_one_by_one)
 *
 */
const NPCDataListItemLayout = ({ item, index, extra }) => {
  return (
    <DataListItem itemData={item} itemStyle={itemStyle(item, index, extra)} />
  );
};
 ...
 *
 */
const DataListItemLayout = ({ item, index, extra }) => {
  return (
    <DataListItem itemData={item} itemStyle={itemStyle(item, index, extra)} />
  );
};
...
const getDataListStyle = (props) => {
  return {
    ...
    renderDataListItem: ({ item, index }) => {
      return <DataListItemLayout item={item} index={index} extra={props} />;
    },
    ...
    renderNPCDataListItem: ({ item, index }) => {
      return <NPCDataListItemLayout item={item} index={index} extra={props} />;
    },
  ...
  return {
    ...
    onItemLongPress: () => {
      ...
      OuiOuNon({
        ...
        onYesPressed: () => {
          SqliteReduxCamouflage.DeleteSpecificRowsFromDB({
            rowName: "uniqueId",
